...
def pre_process(input_file_path):
    #read data into rdd
    tfRDD = sc.textFile(input_file_path).map(lambda x: x.split(','))
    first = tfRDD.take(1)

    tf = tfRDD.filter(lambda x: x != first[0])\
                .map(lambda x: (x[0][1:6] + x[0][8:-1] + '-' + x[1][1:-1].lstrip('0'), x[5][1:-1].lstrip('0')))
    
    return tf


def write_data(data, preprocess_file_path):
    res = data.collect()
...
if __name__ == '__main__':
    ...
    #read data into rdd
    tf = pre_process(input_file_path)
    
    # write preprocess data into csv file:
#     preprocess_file_path = ""
#     write_data(tf, preprocess_file_path)
    
    # build basket
    basket = tf.groupByKey().mapValues(set).filter(lambda x: len(x[1]) > k)
